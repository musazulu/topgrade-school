...
def staff_list(request):
    """Staff listing view"""
    staff = StaffMember.objects.filter(is_active=True).order_by(
        "staff_type", "display_order"
    )
...
def subjects(request):
    ...
    try:
        ...
        for subject in subjects_list:
            print(
                f"  - ID: {subject.id}, Name: {subject.name}, Category: {subject.category}, Display: {subject.get_category_display()}"
            )
        ...
        for subject in subjects_list:
            ...
            print(
                f"🔍 DEBUG: Processing subject '{subject.name}' -> category_display: '{category_display}'"
            )
        ...
        print(
            f"🔍 DEBUG: Final subjects_by_category keys: {list(subjects_by_category.keys())}"
        )
...
def gallery(request):
    """Gallery page view"""
    images = GalleryImage.objects.filter(is_active=True).order_by(
    ...
    return render(
        request, "school/gallery.html", {"images_by_category": images_by_category}
    )
...
def contact(request):
    ...
    form = ContactForm()
    return render(
        request, "school/contact.html", {"school_info": school_info, "form": form}
    )
...
def enrollment_form(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "school/enrollment_success.html",
                {
                    "email": enrollment.email,
                    "application_id": enrollment.id,
                    "submission_date": enrollment.application_date.strftime(
                        "%B %d, %Y at %I:%M %p"
                    ),
                },
            )
...
def uniforms(request):
    """Display school uniforms page"""
    return render(request, "school/uniforms.html")
